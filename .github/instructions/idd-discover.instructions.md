...
2. If the target issue carries the configured roadmap label or an
   `dantalion-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `dantalion-blocked-by` markers) and its
...
`dantalion-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`dantalion-roadmap-id` marker. Retro-label a legacy
...
`<!-- dantalion-roadmap-id: ... -->` marker are
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- dantalion-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- dantalion-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- dantalion-effort: S|M|L -->` footer
