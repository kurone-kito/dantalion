...
Example: repo `dantalion`, branch `issue/123-add-foo` → path
...
**Editing a docs/instructions file**: dantalion does not ship a local
generated mirror for the imported IDD surface. Edit the checked-in
consumer file directly, preserve its pinned upstream provenance and
repository-local substitutions, and keep documentation-only refreshes
within their owning issue. Do not silently regenerate these files from an
untracked or floating upstream branch.
