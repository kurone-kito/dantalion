...
4. **Prefix-first**: resolve the target repository's marker prefix
   ...
   default to an upstream source prefix in this installed bundle; this
   checkout resolves its configured prefix as `dantalion`.
...
- For this dantalion checkout, use `.github/idd/config.json`,
  `docs/idd-workflow.md`, and the routed files under
  `.github/instructions/` for repository-local policy and execution
  references. Do not rely on source-repository-relative maintenance docs
  or a source-repository journal being present in this installed bundle.
...
- Resolve the target repository's marker prefix before emitting any
  authoring marker; never assume an upstream source prefix in an
  installed bundle (the prefix-first rule). In this checkout the
  configured prefix is `dantalion`.
