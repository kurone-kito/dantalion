...
or copied outside the upstream source repository. It mirrors the
canonical contract from `idd-skill` at the reviewed commit
`5c2704a1b50901f29d87865002047b1eb491865e`; an installed copy should
use its target repository's local policy and execution docs for any
...
- Use the prefix documented by the target repository's onboarding or
  IDD instructions.
- In this dantalion checkout the configured prefix is `dantalion`.
  Installed bundles must not default to an upstream source prefix; use
  the value actually configured by the target repository.
...
4. When an issue drafts a **template resync or reimport** (pulling a
   ...
   `docs/customization.md` section or, from an upstream `idd-skill`
   source checkout, `idd-template/ONBOARDING.md`'s "Re-importing"
   ...
   2026-08-12/13 on an
   adopter repository, `setup.ubuntu`.
...
[High-contention shared-file overlap](https://github.com/kurone-kito/idd-skill/blob/5c2704a1b50901f29d87865002047b1eb491865e/docs/policy-constants.md#high-contention-shared-files)).
...
`owner/repo#N` shorthand (e.g. `example-org/example-repo#4321`) is also
...
an upstream source prefix, which produces a
...
- **Soft tie-breaker, never a gate.** Effort only reorders candidates
  **within** a single suitability-score band, after the score and
  optional session-offset rules and **before** the lowest-issue-number
  tie-break.
...
[Upstream-candidate escalation](https://github.com/kurone-kito/idd-skill/blob/5c2704a1b50901f29d87865002047b1eb491865e/idd-template/.github/instructions/idd-overview-appendix.instructions.md#upstream-candidate-escalation)
...
- **Per-target ownership.** The configured label is a shared
  ...
  generation. During the owning set's own Stage 2 (observed in the
  upstream implementation), the heartbeat renewal and the pre-removal
